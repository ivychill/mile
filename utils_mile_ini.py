...
class MI_Estimator(torch.nn.Module):
    def __init__(self, network, Nbatch, Nsample, f_divergence_type='GAN_JS',pair_type=0, varitional_type=1, GMM_model=None):
...
class MI_LogDet_Estimator(torch.nn.Module):
    def __init__(self, beta,Kmax,method='kmeans'):
    ...
    def covariance2entropy_estimator(self, x):
        ...
        if K > 1:
            ...
            H_l = self.H_lower(w,mu,cov)
            Ent = 0.5*(self.H_lower(w,mu,cov)+self.H_upper(w,mu,cov,self.beta))
        else:
            cov = torch.cov(x.T)
            Ent = d / 2 * (torch.log(torch.tensor(2 * math.pi)) + 1) + 0.5 * torch.logdet(self.beta * torch.eye(d) + cov)
            H_u=Ent
            H_l=Ent
        return Ent, H_l, H_u
    ...
    def MILE_estimate(self, x, y):
        ...
        if self.method == 'Kmeans':
            ...
            Hxy = self.covariance2entropy_estimator_SoftAssign(torch.concatenate((x, y), dim=1))
        # use H(x|y)=H(x,y)-H(x)
        # use MI(x,y)=H(x)+H(y)-H(x,y)
        MIx_y_bias = Hx + Hy - Hxy
        MIx_y_unbias = Hx + Hy - Hxy + 2*bias_est_1-bias_est_2
        MIx_y_l = Hx_l+Hy_l-Hxy_l
        MIx_y_u = Hx_u+Hy_u-Hxy_u

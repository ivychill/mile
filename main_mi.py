...
def smile_estimator(critic_params, data_params, mi_params, opt_params, df_smile, df_stat, **kwargs):
    ...
    def stat(df_stat, true_mi, rho, data_params, mi_params, clip=None):
        if mi_params['critic'] == 'conditional':
                critic_ = CRITICS['conditional'](rho=rho).cuda()
        else:
            critic_ = critic
        ...
        df_stat.at[('bias', estimator), true_mi] = round(bias, 4)
        df_stat.at[('var', estimator), true_mi] = round(var, 4)
        df_stat.at[('mse', estimator), true_mi] = round(mse, 4)
    ...
    estimates = []
    for index in range(opt_params['iterations']):
        mi = train_step(rhos[index], data_params, mi_params)
        mi = mi.detach().cpu().numpy()
        estimates.append(mi)
        
        # save
        critic_name = mi_params['critic']
        # df_smile.at[critic_name, index] = mi
        df_smile.at[index, critic_name] = mi
        step_per_mi = opt_params['iterations']//len(opt_params['mis'])
        if critic_name == 'concat' and index % step_per_mi == step_per_mi - 1:
            stat(df_stat, mis[index], rhos[index], data_params, mi_params, **kwargs)
    
    return np.array(estimates)


def mile_estimator(mile_name, data_params, opt_params, df_mile, df_stat):
    ...
    def stat(data_params, opt_params):
        for true_mi in opt_params['mis']:
            ...
            df_stat.at[('bias', mile_name), true_mi] = round(bias, 4)
            df_stat.at[('var', mile_name), true_mi] = round(var, 4)
            df_stat.at[('mse', mile_name), true_mi] = round(mse, 4)
    ...
    for index in range(opt_params['iterations_mile']):
        ...
        df_mile.at['unbias', index] = kmeans_5_unbias
        df_mile.at['lower', index] = kmeans_5_lower
        df_mile.at['upper', index] = kmeans_5_upper
...
def plot(opt_params, smile_dict, mile_dict, file):
    ncols = 5
    nrows = 1
    EMA_SPAN = 200
    ...
    estimator = 'smile'
    for index, clip in enumerate([5.0, None]):
        if clip is None:
            key = estimator
        else:
    ...
    plt.ylabel('MI (nats)')
    plt.xlabel('Steps')
    plt.plot(X, np.squeeze(np.array(mile_dict['kmeans_5_upper'])), label='$MILE$'+' '+'$\widehat{I}^u$')
    plt.plot(X, np.squeeze(np.array(mile_dict['kmeans_5_unbias'])), label='$MILE$'+' '+'$\widehat{I}^m$')
    plt.plot(X, np.squeeze(np.array(mile_dict['kmeans_5_lower'])), label='$MILE$'+' '+'$\widehat{I}^l$')
    plt.plot(mi_true, color='k', label='True MI')
    plt.axhline(np.log(64), color='k', ls='--', label='log(bs)')
    plt.legend(loc="upper left")
...
if __name__ == "__main__":
    ...
    set_logger(logger, log_dir / "mi.log")
    ...
    excel_path_stat = excel_dir / f'stat_{dim}.xlsx'
    writer_stat = pd.ExcelWriter(excel_path_stat, engine='xlsxwriter')
    excel_path_mile = excel_dir / f'mile_{dim}.xlsx'
    writer_mile = pd.ExcelWriter(excel_path_mile, engine='xlsxwriter')
    excel_path_smile = excel_dir / f'smile_{dim}.xlsx'
    writer_smile = pd.ExcelWriter(excel_path_smile, engine='xlsxwriter')
    ...
    for y_transform in y_transforms:
        ...
        df_stat = pd.DataFrame([], index=midx, columns=mi_seeds)
        df_mile = pd.DataFrame([])
        mile_dict = mile_estimator(mile_name, data_params, opt_params, df_mile, df_stat)
        df_mile.to_excel(writer_mile, sheet_name=y_transform)
        
        smile_dict = dict()
        df_smile = pd.DataFrame([])
        for critic_type in ['separable', 'concat']:
            smile_dict[critic_type] = dict()
            for estimator in estimators:
                algorithm = find_name(estimator)
                mi_params = dict(estimator=estimator, critic=critic_type, baseline='unnormalized')
                mis = smile_estimator(critic_params, data_params, mi_params, opt_params, df_smile, df_stat)
                smile_dict[critic_type][f'{estimator}'] = mis
                df_smile.to_excel(writer_smile, sheet_name=f'{algorithm}_{y_transform}')
            ...
            for clip in clips:
                ...
                algorithm = find_name(key)
                mi_params = dict(estimator=estimator, critic=critic_type, baseline='unnormalized')
                mis = smile_estimator(critic_params, data_params, mi_params, opt_params, df_smile, df_stat, clip=clip)
                smile_dict[critic_type][f'{estimator}_{clip}'] = mis
                df_smile.to_excel(writer_smile, sheet_name=f'{algorithm}_{y_transform}')
        
        file = plot_dir/f'mi_{y_transform}_{dim}.png'
        plot(opt_params, smile_dict, mile_dict, file)
        df_stat.to_excel(writer_stat, sheet_name=y_transform)
